...
class CrossViewAttention(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.cfg = cfg
        self.in_channels = in_channels # number of channels in the input feature maps
        self.num_heads = cfg.NETWORK.CROSS_ATT_NUM_HEADS # number of attention heads
        self.reduced_channels = in_channels // cfg.NETWORK.CROSS_ATT_REDUCTION_RATIO  # channel dimension is reduced for efficiency
        self.attention_spatial_downsample_ratio = cfg.NETWORK.ATT_SPATIAL_DOWNSAMPLE_RATIO # optional spatial downsampling of the feature maps
        ...
        # This is a depthwise convolution : each input channel is convoluted with its own filter.
        # efficiently reduces the spatial dimensions of the input features that will be used to compute Query (Q), Key (K), and Value (V) matrices
        ...
        # QKV Projection
        # This 1x1 convolution projects the input features into three separate feature maps for Query (Q), Key (K), and Value (V).
        self.qkv_conv = nn.Conv2d(in_channels, 3 * self.reduced_channels, kernel_size=1)
        # Applied to the attention scores to normalize them into a probability distribution, ensuring they sum to 1 along the 
        # dimension over which attention is computed (in this case, across views)
        self.softmax = nn.Softmax(dim=-1)

        # Projection Back to in_channels
        # After the attention mechanism, the attended (weighted) values, which have reduced_channels, are projected back to the original in_channels.
        ...
        # This FFN allows the model to perform additional non-linear transformations on the attended features.
        ...
        self.batch_norm = nn.BatchNorm2d(in_channels) # Normalizes the features to stabilize and accelerate training
        self.dropout = nn.Dropout(0.1) # Regularization technique to prevent overfitting
    ...
    def forward(self, x):
        ...
        # QKV Projection
        # x_for_qkv features are projected into concatenated Query, Key, and Value features qkv
        qkv = self.qkv_conv(x_for_qkv)                                 # [batch_size * n_views, 3 * reduced_channels, new_h, new_w]
        # qkv tensor is split along the channel dimension into individual q, k, and v tensors.
        ...
        # THIS IS CROSS-VIEW ATTENTION
        # Softmax is applied across the last dimension (n_views), meaning for each query view, its attention weights to all other views sum to 1.
        ...
        # The attention scores are multiplied with the value features. This essentially performs a weighted average of 
        # the value features from all views for each query view, based on the computed attention scores.
        ...
        # If spatial downsampling was applied earlier, the features are now upsampled back to the original height, width.
        if self.downsample_qkv:
        ...
        # Residual Connection
        # Reshaped back to per-view structure.
        attended_values = attended_values.view(batch_size, n_views, self.in_channels, height, width)
        # Residual (skip) connection adds the original input x back to the attended features.
        # Helps preserve information from the original features and aids training stability.
        attended_values = attended_values + x  # Skip connection

        # Feed-Forward Network
        # FFN is standard practice in Transformer blocks for further refinement and regularization.
        ...
        # BatchNorm and Dropout for model stability 
        attended_values_flat = attended_values.view(batch_size * n_views, self.in_channels, height, width)
